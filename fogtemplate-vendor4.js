...
// ---- NAVIGATION ELEMENTS (IN MENU SLIDE FROM RIGHT) 	
...
// ------ NAVIGATION ELEMENTS UNDERLINE (IN MENU SLIDE FROM RIGHT) 
// Mouseenter function
...
// --- NAVIGATION ELEMENTS CHANGE COLOR (IN MENU SLIDE FROM RIGHT)
...
// ---- CLICK ON BOX TO RESIZE OR FULLSCXREEN (FLIP GSAP JE ČINI MI SE BOLJA ALTERNATIVA)
...
var buttone = document.querySelector("#boxxx");
var box = document.querySelector("#boxxx");

buttone.addEventListener("click", onClick);
...
function flip(elements, changeFunc, vars) {
  ...
  copy.x = i => after[i].x + (before[i].l - after[i].l) + ((before[i].w / after[i].w) * after[i].sx - after[i].sx) * after[i].origin[0];
  copy.y = i => after[i].y + (before[i].t - after[i].t) + ((before[i].h / after[i].h) * after[i].sy - after[i].sy) * after[i].origin[1];
  copy.scaleX = i => after[i].sx * (before[i].w / after[i].w);
  copy.scaleY = i => after[i].sy * (before[i].h / after[i].h);
